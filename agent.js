...
module.exports = agent => {
  const {
    host, port, username, password, database, connectUri,
  } = agent.config.sequelizePlus;


  const creatDateBaseSql = `CREATE DATABASE IF NOT EXISTS ${database} default charset utf8 COLLATE utf8_general_ci`;

  const connection = mysql.createConnection(
    connectUri != null && typeof connectUri === 'string'
      ? connectUri : { host, port, user: username, password }
  );

  connection.connect();
  connection.query(creatDateBaseSql, err => {
    if (err) {
