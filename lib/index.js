'use strict';
/**
 * @description Database connection configuration load
 * @author Taylor
 * @time 2019-11-11
 * @address wuhan
 */
...
module.exports = app => {
  ...
  function loadDatabase(config = {}) {
    if (typeof config.ignore === 'string' || Array.isArray(config.ignore)) {
      app.deprecate(`[egg-sequelize-plus] if you want to exclude ${config.ignore} when load models, please set to config.sequelize.exclude instead of config.sequelize.ignore`);
